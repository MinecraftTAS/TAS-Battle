	...
	public static void installFFmpeg(final Minecraft client) {
		...
		new Thread(() -> {
			try {
				...
				VideoUpspeederScreen.installingProgress = "Downloading https://data.mgnet.work/ffmpeg.zip";
				...
				e.printStackTrace();
				Minecraft.crash(new CrashReport("Downloading of FFmpeg failed", e));
				client.stop();
			}
		}).start();
	}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					try {
						if (Minecraft.getInstance().screen instanceof VideoUpspeederScreen) Minecraft.getInstance().setScreen(new VideoUpspeederScreen());
					} catch (Exception e) {
