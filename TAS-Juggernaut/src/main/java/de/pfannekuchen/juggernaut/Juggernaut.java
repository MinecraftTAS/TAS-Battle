 ...
 * @author Pancake
 */
public class Juggernaut extends JavaPlugin implements PluginMessageListener {

	private static Juggernaut instance;
	public static Juggernaut instance() { return instance; }
			...
			new Thread(() -> {
				while (true) {
					...
					for (Player p : Bukkit.getOnlinePlayers()) {
						p.sendPluginMessage(Juggernaut.instance, "tickratechanger:data2", ByteBuffer.allocate(20).putInt(1).putInt(Game.alivePlayers.size()).putInt(12).putLong(Instant.now().toEpochMilli()).array());	
					}
				}
			}).start();
		} catch (Exception e) {
	...
	public static void updateTickrate(float float1) throws Exception {
		...
		for (Player player : Bukkit.getOnlinePlayers()) {
			player.sendPluginMessage(Juggernaut.instance, "tickratechanger:data", ByteBuffer.allocate(4).putFloat(float1).array());
		}
	}
	...
	public void onPluginMessageReceived(String channel, Player player, byte[] message) {
		if (channel.equalsIgnoreCase("tickratechanger:data")) {
			...
			player.sendPluginMessage(Juggernaut.instance, "tickratechanger:data", ByteBuffer.allocate(4).putFloat(tickrate).array());
		}
	}
