...
public class MixinTitleScreen extends Screen{
	...
	private void createNormalMenuOptions(int height, int distanceBtn) {
		int i = 48;
		if(TASBattle.isDevEnvironment()) {
			this.addRenderableWidget(Button.builder(Component.translatable("menu.singleplayer"), button -> this.minecraft.setScreen(new SelectWorldScreen(this))).bounds(this.width / 2 - 100, this.height / 4 + i, 200, 20).build());
			i+=24;
		}
		...
		}).bounds(this.width / 2 - 100, this.height / 4 + i, 200, 20).build());
	}
}
