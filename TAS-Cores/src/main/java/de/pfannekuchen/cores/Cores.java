...
/**
 * Main and basically everything of the Cores Plugin
 * @author Pancake
 */
public class Cores extends JavaPlugin implements PluginMessageListener {

	private static Cores instance;
	public static Cores instance() { return instance; }
			...
			new Thread(() -> {
				while (true) {
					...
					for (Player p : Bukkit.getOnlinePlayers()) {
						p.sendPluginMessage(Cores.instance, "tickratechanger:data2", ByteBuffer.allocate(20).putInt(1).putInt(Game.alivePlayers.size()).putInt(12).putLong(Instant.now().toEpochMilli()).array());	
					}
				}
			}).start();
		} catch (Exception e) {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
		try {
			...
			if (command.getName().equalsIgnoreCase("loadkit")) {
				...
				Serialization.deserializeInventory((Player) sender, items, true);
	...
	public static void updateTickrate(float float1) throws Exception {
		...
		for (Player player : Bukkit.getOnlinePlayers()) {
			player.sendPluginMessage(Cores.instance, "tickratechanger:data", ByteBuffer.allocate(4).putFloat(float1).array());
		}
	}
	...
	public void onPluginMessageReceived(String channel, Player player, byte[] message) {
		if (channel.equalsIgnoreCase("tickratechanger:data")) {
			...
			player.sendPluginMessage(Cores.instance, "tickratechanger:data", ByteBuffer.allocate(4).putFloat(tickrate).array());
		}
	}
